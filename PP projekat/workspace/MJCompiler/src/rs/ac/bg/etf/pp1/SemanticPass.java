	...
	public void visit(DesignatorOpStmt desgStmt) {
		...
		if(desOp instanceof DesignatorInc || desOp instanceof DesignatorDec) {
			...
			if(des.getType().getKind() != Struct.Int && des.getType().getElemType() != Tab.intType ) {
				report_error("Greska:Promenljiva mora biti tipa int pri INC i DEC", desgStmt);
				return;
			}
		}
...
//				return;
//			}
			if(e == null || e == Tab.noType || !e.assignableTo(des.getType())) {
				report_error("Greska: Nekompatibilni tipovi pri dodeli vrednosti!", desgStmt);
				return;
			}
		}
...
//		}	
	...
	public void visit(ConstBool constBool) {
		...
		constBool.obj.setAdr(val.equals("true") ? 1 : 0);
	}
	...
	public void visit(ConstDeclPart constDeclPart) {
		...
		constDeclPart.obj.setAdr(constDeclPart.getConstValue().obj.getAdr());
		constDeclCount++;
	}
...
//			return;
//		}
		if(factListSt != Tab.intType || factSt != Tab.intType) {
			report_error("Greska: Sabiranje nevalidnih tipova, mora int", factorList);
			return;
		}
...
//			return;
//		}
		if(termListSt != Tab.intType || termSt != Tab.intType) {
			report_error("Greska: Sabiranje nevalidnih tipova, mora int", term);
			return;
		}
	...
	public void visit(ExprRelopMulti cond) {
		Struct est1 = cond.getExpr1().struct;
		Struct est2 = cond.getExpr().struct;
	...
	public void visit(ReadStmt readStmt) {
		...
		if(rst.getKind() != Struct.Int && rst.getKind() != Struct.Char && rst.getKind() != Struct.Bool) {
			report_error("Greska: Designator mora biti int, char ili bool", readStmt);
			return;
		}
	}
